...
function obtainable(d: TechDef): boolean {
  ...
  // elsewhere. Counted as "start", such recipes dragged every surgery mod down.
...
function gatesOf(d: TechDef): { name: string; from: string | null }[] {
  ...
  if (building) {
    ...
    return (d.Refs?.Research ?? []).filter((name) => !recipe.has(name)).map((name) => ({ name, from: null }));
  }
...
export function techItems(defs: TechDef[], vanillaResearch: Record<string, string | null>): TechItem[] {
  ...
  for (const d of defs) {
    ...
    const gates = (product ?? gatesOf(d)).map((g) => {
    ...
    const step: TechStep | null = objectWins ? own : byResearch ?? (gates.length > 0 ? null : "start");
    items.push({ ...base, step, gates, objectWins });
  }
  return items.sort((a, b) => (a.step === null ? 1 : 0) - (b.step === null ? 1 : 0) || rank(a.step ?? "start") - rank(b.step ?? "start"));
}
...
export function proposedFix(i: TechItem): { from: TechLevel | null; to: TechLevel } | null {
  if (i.defType !== "ThingDef" || !i.defName || i.step === null || i.step === "start") return null;
